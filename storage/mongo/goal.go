...
func (g *goalRepo) Create(ctx context.Context, req *pb.CreateGoal) (*pb.Goal, error) {
	model := models.CreateGoal{
		...
		Name:          req.GetName(),
		TargetAmount:  float64(req.GetTargetAmount()),
		CurrentAmount: float64(req.GetCurrentAmount()),
	...
	return &pb.Goal{
		...
		UserId:        model.UserID,
		Name:          model.Name,
		TargetAmount:  float32(model.TargetAmount),
		CurrentAmount: float32(model.CurrentAmount),
